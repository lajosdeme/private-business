...
contract EmployerScroll is Ownable {
    ...
    event AddVerifiedEmployee(address indexed employee);
    ...
    function addVerifiedEmployee(address employee) external onlyOwner() {
        verifiedEmployees[employee] = true;

        emit AddVerifiedEmployee(employee);
    }

    function isVerifiedEmployee(address user) public view returns (bool) {
        return verifiedEmployees[user];
    }
